...
public class RobotMovement : MonoBehaviour
{
    ...
    {
        Debug.Log("picture");
        ChangePicture(image, test_image);
    }
    ...
    private static void ChangePicture(RawImage image, byte[] byteArr)
    {
        ...
        tex.LoadImage(byteArr);
        // Assign texture to renderer's material.
        image.material.mainTexture = tex;

        //byte[] jpgBytes = ImageConversion.EncodeArrayToJPG(tex.GetRawTextureData(), tex.graphicsFormat, 100, 100);
        //File.WriteAllBytes("Assets/Scenes/TestImage.jpg", jpgBytes);
...
public class RobotMovement : MonoBehaviour
{
    ...
    async private void HandleMovement(string cmd)
    {
        ...
        using (var httpClient = new Windows.Web.Http.HttpClient())
        {
            ...
            try
            {
                ...
                {
                    Debug.LogError("Error Sending Robot Command");
                }
            }
    ...
    async public void PictureButtonPressHandler(RawImage image)
    {
        ...
        using (var httpClient = new Windows.Web.Http.HttpClient())
        {
            ...
            try
            {
                var buff = await httpClient.GetBufferAsync(uri);
                Windows.Storage.Streams.DataReader dataReader = Windows.Storage.Streams.DataReader.FromBuffer(buff);
                byte[] byteArr = new byte[buff.Length];
                dataReader.ReadBytes(byteArr);
                ChangePicture(image, byteArr);
            }
            catch (System.Exception ex)
            {
                Debug.LogError(ex.Message);
