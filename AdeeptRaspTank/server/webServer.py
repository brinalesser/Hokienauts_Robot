...
async def check_permit(btsocket):
    while True:
        recv_str = await btsocket.recv(BUFSIZ)
        cred_dict = recv_str.split(":")
        if cred_dict[0] == "admin" and cred_dict[1] == "123456":
            response_str = "congratulation, you have connect with server\r\nnow, you can do something else"
            await btsocket.send(response_str)
            return True
        else:
            response_str = "sorry, the username or password is wrong, please submit again"
            await btsocket.send(response_str)

async def recv_msg(socket):
    ...
    while True: 
        ...
        data = ''
        data = await btsocket.recv(BUFSIZ)
        try:
        ...
        await btsocket.send(response)

async def main_logic(btsocket, path):
    await check_permit(btsocket)
    await recv_msg(btsocket)
...
if __name__ == '__main__':
    ...
    while  1:
        try:
            start_server = bluetooth.BluetoothSocket( bluetooth.RFCOMM )
            server_sock.bind((HOST,PORT))
            server_sock.listen(1)
            btsocket,address = server_sock.accept()
